        ...
        try
        {
            switch (action)
            {
                ...
                case "Cloud_Doc":
                    ...
                    uploadPath = context.Server.UrlDecode(SiteConfig.SiteOption.UploadDir + mu.UserName + value + "/");
                    break;
            ...
            else if (action.Equals("Cloud_Doc"))
            {
                ...
                {
                    string icourl= SiteConfig.SiteOption.UploadDir + buser.GetLogin().UserName+"/ico" + value + "/";
                    if (!Directory.Exists(function.VToP(icourl))) { Directory.CreateDirectory(function.VToP(icourl)); }
                    ImgHelper imghelp = new ImgHelper();
                    imghelp.CompressImg(file, 100,icourl+file.FileName);
                }
