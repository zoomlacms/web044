<%@ page language="C#" autoeventwireup="true" inherits="Manage_I_Content_NodeManage, App_Web_chetbuey" masterpagefile="~/Manage/I/Default.master" enableEventValidation="false" viewStateEncryptionMode="Never" %>
...
<script type="text/javascript">
	...
	var trCode = "<tr onclick=\"getNodeList(this,'nodeList','{nid}');\" class=\"tdbg list{pid}\" id=\"list{nid}\" name=\"list{pid}\" align=\"center\" onmouseover=\"this.className='list{pid}'\" onmouseout=\"this.className='tdbg list{pid}'\"  state=\"1\" ondblclick=\"showlist({nid})\" >"
	...
	<asp:Repeater ID="Repeater1" runat="server" EnableViewState="false">
		<ItemTemplate>
			<tr onclick="getNodeList(this,'nodeList','<%#Eval("NodeID")%>');" class="tdbg list<%# Eval("ParentID") %>" id='list<%#Eval("NodeID")%>' name="list<%#Getprname(Eval("NodeID"))%>"
				align="center" onmouseover="this.className='list<%# Eval("ParentID") %>'"
				...
				<td class="text-left">
					<%# GetIcon(Eval("NodeName","{0}"),Convert.ToInt32(Eval("NodeID")),Eval("Depth","{0}"),Eval("NodeType","{0}"))%>
				</td>
				<td><strong><%# GetNodeType(Eval("NodeType","{0}"))%></strong></td>
				<td><%# GetOper(Eval("NodeID","{0}"),Eval("NodeType","{0}"))%></td>
			</tr>
		</ItemTemplate>
	</asp:Repeater>
</table>
