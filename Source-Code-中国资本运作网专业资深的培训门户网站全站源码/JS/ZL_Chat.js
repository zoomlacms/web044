...
//添加用户进入在线用户
ZL_Chat.prototype.AddOnline=function(uname){
    $.ajax({
        type: "Post",
        url: curChat.ajaxurl,
        ...
        success: function (data) {
            ...
            this.myinfo = JSON.parse(data);
            this.myinfo.CDate = "00:00:00";
            this.BeginInit();
            $("#myfaceimg").attr("src", this.myinfo.UserFace);
            $("#myid_t").val(this.myinfo.UserID);
            $("#myname_t").val(this.myinfo.UserName);
        }
    });//IsLogged end;
}
//获取指定ID的聊天窗体,如果无则新建
